...
func (fbha *FreeingBumpHeapAllocator) growHeap(mem Memory, numPages uint32) error {
	err := mem.Grow(numPages)
	if err != nil {
		return err
	}
...
func (fbha *FreeingBumpHeapAllocator) Allocate(mem Memory, size uint32) (uint32, error) {
	...
	if (itemSize + fbha.totalSize + fbha.ptrOffset) > fbha.maxHeapSize {
		...
		err := fbha.growHeap(mem, pagesNeeded+1)
		if err != nil {
	...
	if item := fbha.heads[listIndex]; item != 0 {
		...
		fourBytes := fbha.getHeap4bytes(mem, item)
	...
	for i := uint32(1); i <= 8; i++ {
		fbha.setHeap(mem, ptr-i, 255)
	}
	fbha.setHeap(mem, ptr-8, uint8(listIndex))
...
func (fbha *FreeingBumpHeapAllocator) Deallocate(mem Memory, pointer uint32) error {
	ptr := pointer - fbha.ptrOffset
	if ptr < 8 {
		return errors.New("invalid pointer for deallocation")
	}
	listIndex := fbha.getHeapByte(mem, ptr-8)
	...
	fbha.setHeap4bytes(mem, ptr-8, bTail)
...
func (fbha *FreeingBumpHeapAllocator) setHeap(mem Memory, ptr uint32, value uint8) {
	mem.Data()[fbha.ptrOffset+ptr] = value
}

func (fbha *FreeingBumpHeapAllocator) setHeap4bytes(mem Memory, ptr uint32, value []byte) {
	copy(mem.Data()[fbha.ptrOffset+ptr:fbha.ptrOffset+ptr+4], value)
}
func (fbha *FreeingBumpHeapAllocator) getHeap4bytes(mem Memory, ptr uint32) []byte {
	return mem.Data()[fbha.ptrOffset+ptr : fbha.ptrOffset+ptr+4]
}

func (fbha *FreeingBumpHeapAllocator) getHeapByte(mem Memory, ptr uint32) byte {
	return mem.Data()[fbha.ptrOffset+ptr]
}
